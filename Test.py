...
import BGResource_rc, IconResource_rc
...
class Station(QMainWindow, ui):
    ...
    def fetchPass(self):
        ...
        file = open('SQL Dump/dbInfo.txt','r')
        password =  file.read()
        file.close()
        return password
    ...
    def handleButton(self):
        ...
        self.pushButton_67.clicked.connect(self.openConfig)
        ...
        self.pushButton_68.clicked.connect(self.sqlConfiguration)
        self.pushButton_69.clicked.connect(self.openLogin)
    ...
    def fetchSignalName(self, rRoad):
        ...
        pwd = self.fetchPass()
        sRoad = 'Road ' + rRoad
        self.con = sql.connect('localhost', 'root', pwd, 'station')
    ...
    def setUpLSS(self, delay, aspect):
    
        setS2A = threading.Timer(delay, self.setSignalS2ALSS, [aspect])
        setS2A.start()

    def resetUpHome(self, delay, aspect):
    ...
    def setDnLSS(self, delay, aspect):
    
        setS2A = threading.Timer(delay, self.setSignalS1ALSS, [aspect])
        setS2A.start()

    def resetDnHome(self, delay, aspect):
    ...
    def logSignalDetails(self, rDirection, tName, cTime):
    ...
    def exportTicket(self):
        ...
        try:
            ...
            tData = self.cur.fetchone()  # @UnusedVariable
    ...
    def sqlConfiguration(self):
        ...
        pwd = self.lineEdit_25.text()
        
        try:
            
            file = open('SQL Dump/dbinfo.txt','w')
            file.write(pwd)
            self.statusBar().showMessage('SQL password updated')
            ...
            self.statusBar().showMessage('Unable to update SQL password')
            self.statusBarClear()
